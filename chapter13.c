...
int main (void){ 13-01-03 문제 풀이 2021.10.07 : ptr 포인터 변수를 arr[4] 주소로 선언하고
                                                 ptr 포인터 변수에 저장된 값을 감소시키는 형태로 총 합을 구하라

    int arr[5] = {1, 2, 3, 4, 5};
    int *ptr = &arr[4];
    int total = 0;

    for(int i = 0; i < 5; i++) 포인터 변수에 저장된 값을 감소시키면서 총 합을 구하는 코드
        total += *(ptr-i);
